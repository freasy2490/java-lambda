...
public class CallTipoAPI {
	...
	public CallTipoAPI(String API, String applnum) throws Exception {
		...
		JSONObject tw_patent_rights = jsonObject.getJSONObject("tw-patent-rights" + patent_type);
		if (tw_patent_rights.getInt("-page-count")>0) {
	...
	private static String sendGet(String API, String applnum) throws Exception {
		...
		String url = "https://tiponet.tipo.gov.tw/OpenDataApi/OpenData/API/PatentRights?format=json&orderby=appl-no&tk=ggT4gWtb&applclass="
				+ API + "&top=5000&applno=" + applnum;
		URL obj = new URL(url);
